...
!>  \version      5.11
!>  \date         early eighties. 19 April 2024.
...
!     estimates the equilibrium volume
      ...
      above = 1.5*vmin
      do n = 1,npt
        if(xar(n) .gt. below .and. xar(n) .lt. vmin-eps) below = xar(n)
...
!   stores for plot subroutine
  ...
  if(nstr > 1) then
    write(6,*)
    write(6,*) '  Enter maximum pressure (GPa) and number of points'
    write(6,*) '  For the plots as a function of pressure'
    write(6,*) '  Suggestion:  100.0   50'
    write(6,*)
    ...
    write(ioreplay,'(3x,f17.8,3x,f17.8,10x,"volume energy for point ",i5)')   &
                alat(mm), yar(mm),mm
...
! writes tape for gnuplot plotting software
...
! counters
  ...
  do j = 1,nstr
    ...
    write(iocom,'("''eofv_dat.gp'' using 1:2 index ",i3," w p ls ",      &
        &     i3," ti ''",a10," calculated'', \")')   j-1, j,label(j)
  enddo
  ...
  do j = 1,npts
    ...
    if(jmin /= jmp) then
      ...
      write(iocom,'("''eqst_dat.gp'' using 1:2 index ",i3,               &
          &    " w li ls ",i3," ti ''",a10,"'', \")') jmp-1, jmp,label(jmp)
      write(iodat,*)
      write(iodat,*)
   endif
...
! counters
  ...
  do iter = 1,ITMAX
    ...
    if(abs(e) > tol1) then
      ...
      etemp = e
      e = d
      if(abs(p) >= abs(q*etemp/2) .or. p <= q*(a-x) .or. p >= q*(b-x)) then
        ljump  =  .true.
      else
        d = p/q
        u = x+d
