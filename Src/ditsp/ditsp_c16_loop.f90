...
! state at return from subroutine
  ...
  do n = nconv+1,niter
    ...
    call zaxpy(mtxd,cmplx(-eg(n),ZERO,REAL64),psi(1,n),1,xerror,1)
...
!       removes linear dependent vectors from iteration
        ...
        call grsch_loop_c16(.TRUE., psi, hpsi, mtxd, nconv, niter, irow, mxddim)

        call grsch_shift_c16(psi, mtxd, nconv, niter, irow, ired, mxddim)

        call grsch_shift_c16(hpsi, mtxd, nconv, niter, irow, ired, mxddim)
...
! they are orthogonalized
  ...
  if(.not. lconv .and. (.not. lexit) ) then
    if(ntst > nconv .and. .not. lortho) then
      ...
      call grsch_loop_c16(.TRUE., psi, hpsi, mtxd, nconv, ntst, irow,  mxddim)
...
!   relaxation step for not yet converged eigenvectors
    ...
    do n = nconv+1,niter
      call zaxpy(mtxd, cmplx(-eg(n),ZERO,REAL64), psi(1,n), 1, hpsi(1,n), 1)
    enddo

    call diag_rq_jac_c16(hpsi(1,nconv+1), eg(nconv+1), hdiag, mtxd,      &
       niter-nconv, mxddim, mxdbnd)
