...
!>     If also=.T. performs the corresponding transformation in hxvec.
       ...
       recursive subroutine cgs_mm_c16(also,xvec,hxvec,mtxd,nvec,        &
     &   irow,xscl,                                                      &
     &   mxddim)
...
!        calls modified gram-schmidt
         ...
         call mod_gs_c16(also,xvec,hxvec,mtxd,nvec,irow,xscl,            &
     &                 mxddim)
...
!        calls the twin for first half of vectors
         ...
         call cgs_mm_2_c16(also,xvec,hxvec,mtxd,nn,irow,xscl,            &
     &   mxddim)
...
!        repeat for stability
         ...
         do n=1,nn2
           ...
           if( xsum/xnorm(nn+n) > 0.9999) lrepeat = .TRUE.
         enddo
...
!        calls the twin for second half of vectors
          ...
          call cgs_mm_2_c16(also,xvec(1,nn+1),hxvec(1,nn+1),mtxd,nn2,    &
     &      irow(nn+1),xscl(nn+1),                                       &
     &      mxddim)
