...
!>    sorts an integer array by the heapsort method
!>    adapted from http://rosettacode.org 

      subroutine isort(n,ia,indx)
...
!     input
      ...
      integer, intent(in)        ::  ia(n)                               !<  array to be indexed
...
!       long enough siftdown loop does not exceed ~log(n)/log(2)
        ...
        do ic=1,n+5
          ...
          if(ichild <= ibot) then
            if(ichild < ibot) then
              if(ia(indx(ichild)) < ia(indx(ichild+1)))                  &
     &              ichild = ichild + 1
            endif

            if(ia(indxt) < ia(indx(ichild))) then
