...
! input:
  ...
  character(len=13), intent(in)      ::  callcode                        !<  identification of the calling code
...
! open file
  ...
  open(unit = io, file = adjustl(trim(filename)), status='UNKNOWN',      &
            form='FORMATTED')

  write(io,'(72a1)') ("#",j=1,72)
  write(io,'("#",70x,"#")')
  write(io,'("#",6x,"cpw.in input file generated by ",a13, 20x,"#")')    &
                callcode
  write(io,'("#",70x,"#")')
  write(io,'(72a1)') ("#",j=1,72)
...
! Finds total number of atoms
  ...
  write(io,'("StructureSource",15x,a13)') callcode
  write(io,*)
  ...
  idet = 2
  if(callcode == 'pre_relax_vff') idet = 1
