...
!-------------------------------------------------------------------------
      ! initializes interpolation routines
       ...
       if(lso==1) then
        write(*,*) '  calculation will be done using SO perturbation.'       
       endif       
       ...
       write(6,*) '  using interpolation (memory intensive).'
       read(5,*) yesno
       ...
       if(loptical==1) then
        write(*,*) '  calculation will also include dh0drk computation.'       
       endif       
...
!              write(*,'(f8.5,f8.5,f8.5)') rk_grid(1,irk),rk_grid(2,irk),rk_grid(3,irk)              
              irk=irk+1
            enddo
          enddo
        enddo
...
!       nequal = 0
       ...
       write(*,*) '  number k-points:',nkpt
       write(*,*) '  number of atomic orbitals:',mxdorb
       write(*,*) '  number of eigenvalues equal to full precision:',nequal       
       write(*,*) '  total charge:',ztot,nint(ztot)
...
!       endif
       ...
       if (lso==1) then       
        call NonOrthoInterpInit(noiData,lso,loptical,nk1,nk2,nk3,ws_n1,ws_n2,ws_n3,adot,nkpt,2*norb,2*nequal)
        ...
        call NonOrthoInterpInit(noiData,lso,loptical,nk1,nk2,nk3,ws_n1,ws_n2,ws_n3,adot,nkpt,norb,nequal)
...
!-------------------------------------------------------------------------
       if (lso==1) then
          write(*,*) '  Computing full precision eigenvalues ev(i) including Spin-Orbit'          
     ...
     &   mxdtyp,mxdatm,mxdlqp,mxddim,mxdorb,mxdgve)
       ...
       call UnpackEv(IrredBZ, ev_pw_irred, ev_pw, 2*norb, nkpt) 
          ...
          write(*,*) '  Computing full precision eigenvalues ev(i)'
       do irk=1, IrredBZ % nrk
...
!-------------------------------------------------------------------------
       ...
       if (lso==1) then
          write(*,*) '  Computing atomic orbitals: |psi> and overlaps: S=<psi|psi>, H=<psi|H|psi>' 
          write(*,*) '  SO version'         
...
!-------------------------------------------------------------------------
      write(*,*) '  Finishing Setup'          
      call NonOrthoInterpFinishSetup(noiData)
      write(*,*) '  Writing to file for later use.'          
