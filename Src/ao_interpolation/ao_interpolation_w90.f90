...
!  integer itype
!  integer                       ::  info                       !<  if info /=0 subroutine returned with error
...
!        loop over k-points
         ...
         write(*,'(i5," of ", i5, 3f8.5)') irk,nrk2, rkpt(1),rkpt(2),rkpt(3)
     ...
     if (irk==1) then
        do iorb=1, mxdorb
          write(*,'("infolcao",6i6)') iorb, infolcao(1,iorb), infolcao(2,iorb), infolcao(3,iorb) ,infolcao(4,iorb) ,infolcao(5,iorb)
        enddo
     endif
        ...
        if(mtb%lso==1) then
          ...
          call zgemm('N','N',2*mtxd,norb,norb,C_UM,ao_basis_so,2*mxddim, &
      &        mtb%wrk,2*mxdorb,C_ZERO,psi_ao,2*mxddim)

          call SortToGammaSo(psi_ao,isort, mtxd, neig, psi_ao2, 2*mxddim)
     ...
     &   2*mxddim, S12,2*mxdorb,C_ZERO,ao_basis_ortho,2*mxddim)
         ...
         call zgemm('C','N',norb,neig,2*mtxd,C_UM,ao_basis_ortho,2*mxddim,  &
     &   psi_ao,2*mxddim,C_ZERO,prod,2*mxdorb)
     ...
     &         mtb%wrk,mxdorb,C_ZERO,psi_ao,mxddim)
          ...
          call SortToGamma(psi_ao,isort, mtxd, neig, psi_ao2, mxddim)
     ...
     &   mxddim, S12,mxdorb,C_ZERO,ao_basis_ortho,mxddim)
         ...
         call zgemm('C','N',norb,neig,mtxd,C_UM,ao_basis_ortho,mxddim,   &
     &   psi_ao,mxddim,C_ZERO,prod,mxdorb)
...
!        start loop over neighbours
        ...
        do i=1,nntot
          ...
          call hamilt_struct(emax,rkpt2,mtxd2,isort2,qmod,ekpg, lkplusg,    &
     &    ng,kgv,adot,                                                    &
     &    mxdgve,mxddim)
     ...
     &   mxdtyp,mxdatm,mxdlqp,mxddim,mxdorb,mxdgve,mxdlao)
        ...
        if(mtb%lso==1) then
          ...
          call zgemm('N','N',2*mtxd2,norb,norb,C_UM,ao_basis_so,2*mxddim,        &
     &         mtb%wrk,2*mxdorb,C_ZERO,ao_basis_ortho,2*mxddim)

          call SortToGammaSO(ao_basis_ortho,isort2, mtxd2, neig, psi_ao2, 2*mxddim)
     ...
     &         mtb%wrk,mxdorb,C_ZERO,ao_basis_ortho,mxddim)
          ...
          call SortToGamma(ao_basis_ortho,isort2, mtxd2, neig, psi_ao2, mxddim)
...
!          write(*,'("neighbour", 6f8.5, 2i5)') rkpt, rkpt2, mtxd, norb
...
!      call ao_interpolation_write_chk(mtb%fiData, neig)
       ...
       write(*,*) 'W90 interface calculations Done.'

       return
       end subroutine ao_interpolation_w90

subroutine SortToGamma(psi_in,isort_in, mtxd_in, neig, psi_out, mxddim)
  implicit none
...
subroutine SortToGammaSo(psi_in,isort_in, mtxd_in, neig, psi_out, mxddim)
  implicit none
