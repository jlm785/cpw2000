...
!>  \version      5.06
!>  \date         17 December 2019, 22 November 2022.
...
!
!---------------------------------------------------------------------------
SUBROUTINE QE_find_group( nrot, smat, gname, code_group )
  !
  ! USE kinds, ONLY : DP
  !
  IMPLICIT NONE
  ...
  INTEGER, intent(in)                      ::  nrot                      !<  number of rotation matrices
  REAL(DP), intent(in)                     ::  smat(3,3,nrot)            !<  rotation matrices in cartesian coordinates

  INTEGER, intent(out)                     ::  code_group                !<  code that identifies the group
  CHARACTER (LEN=11), intent(out)          ::  gname                     !<  name of the group
  !
  ! ... local variables
  !
  CHARACTER(LEN=11) :: QE_group_name
  INTEGER :: noperation(6), irot, ts, QE_tipo_sym
  !
  ! For each possible group operation the function QE_tipo_sym gives a code
  !   1 identity,
  !   2 inversion,
  ...
  noperation=0
  DO irot = 1, nrot
     ts = QE_tipo_sym(smat(1,1,irot))
     noperation(ts) = noperation(ts)+1
  ENDDO
  !
  IF (noperation(1) /= 1) CALL QE_errore( 'QE_find_group', 'the group has not identity', 1 )
  !
  code_group=0
  !
  IF (noperation(2)==0) THEN
     ...
     CASE DEFAULT
        CALL QE_errore( 'QE_find_group','wrong number of elements', 1 )
     END SELECT
     !
  ELSEIF (noperation(2)==1) THEN
     ...
     END SELECT
  ELSE
     CALL QE_errore( 'QE_find_group', 'too many inversions', 1 )
  ENDIF
  !
  IF (code_group==0) CALL QE_errore( 'QE_find_group', 'incompatible operations', 1 )
  !
  gname = QE_group_name( code_group )
  !
  RETURN
  !
END SUBROUTINE QE_find_group
!
!
!--------------------------------------------------------------------------
FUNCTION QE_group_name( code )
  !--------------------------------------------------------------------------
  ...
  !
  IF (code < 1 .OR. code > 32 ) CALL QE_errore( 'QE_group_name', 'code is out of range', 1 )
  !
  QE_group_name = gname(code)
  !
  RETURN
  !
END FUNCTION QE_group_name
!
!
!--------------------------------------------------------------------------
FUNCTION QE_tipo_sym( s )
  !--------------------------------------------------------------------------
...
!  USE kinds, ONLY : DP
  ...
  IF (ABS(det-1.d0) < eps) THEN
     ...
     IF (ABS(det1) < eps) THEN
        QE_tipo_sym = 4     ! 180 proper rotation
     ELSE
        QE_tipo_sym = 3     ! proper rotation <> 180
     ENDIF
     RETURN
  ENDIF
  !
  ...
  IF (ABS(det+1.d0) < eps) THEN
     ...
     IF (ABS(det1) < eps) THEN
        QE_tipo_sym=5   ! mirror symmetry
     ELSE
        QE_tipo_sym=6   ! improper rotation
     ENDIF
     !
     RETURN
     !
  ELSE
     CALL QE_errore( 'QE_tipo_sym', 'symmetry not recognized', 1 )
  ENDIF
  !
END FUNCTION QE_tipo_sym
!
!--------------------------------------------------------------------------
FUNCTION QE_laue_class( code )
!--------------------------------------------------------------------------
  ...
  DATA  laue  / 2,   2, 16, 16, 27, 18, 19, 20, 25, 22,  &
                ...
                32, 32 /
  !
  IF (code<1 .OR. code>32) CALL QE_errore( 'QE_laue_class', 'code is out of range', 1 )
  !
  QE_laue_class = laue(code)
  !
  RETURN
  !
END FUNCTION QE_laue_class


!----------------------------------------------------------------------------
SUBROUTINE QE_errore( calling_routine, message, ierr )
  !----------------------------------------------------------------------------
  !
  ...
  WRITE( 6, FMT = '(I6)' ) ierr
  WRITE( 6, FMT = '(/,1X,78("%"))' )
  WRITE( 6, FMT = '(5X,"Error in routine ",A," (",A,"):")' ) &
        TRIM(calling_routine)
  WRITE( 6, FMT = '(5X,A)' ) TRIM(message)
  WRITE( 6, FMT = '(1X,78("%"),/)' )
  STOP 1
  !
  RETURN
  !
END SUBROUTINE QE_errore
