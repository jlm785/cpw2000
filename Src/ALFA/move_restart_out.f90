...
!>     for l_bfgs minimization, algorithm is restarted.
       ...
       subroutine move_restart_out(flgcal,                               &
...
!      counters
     ...
     &      form = 'formatted')
       ...
       write(7,'(2x,a6)') flgcal

       write(7,'(9(2x,e24.16))') ((adot(i,j),i=1,3),j=1,3)
       write(7,'(2x,i10)') ntype

       do nt = 1,ntype
         write(7,'(2x,i10,2x,a2,2x,e24.16)') natom(nt),nameat(nt),       &
     &       atmass(nt)

         do i = 1,natom(nt)
           write(7,'(3(2x,e24.16))') (rat(j,i,nt),j=1,3)
         enddo
       enddo

       if(flgcal == 'MICRO ') then

         write(7,'(2x,i10,2x,e24.16)') istmd,tstep
         do nt=1,ntype
           do i=1,natom(nt)
             write(7,'(9(2x,e24.16))') (vat(j,i,nt),j=1,3),              &
     &                   (rat1(j,i,nt),j=1,3),(frc1(j,i,nt),j=1,3)
       ...
       elseif(flgcal == 'LANG  ') then
         ...
         write(7,'(2x,i10,3(2x,e24.16),2x,i10)') istmd,tstep,beta,       &
     &           tempk,iseed
         do nt = 1,ntype
           do i = 1,natom(nt)
             write(7,'(11(2x,e24.16))') (vat(j,i,nt),j=1,3),             &
     &                   (rat1(j,i,nt),j=1,3),(frc1(j,i,nt),j=1,3)
           enddo
         enddo
       ...
       elseif(flgcal == 'VCSLNG' .or. flgcal == 'EPILNG') then
         ...
         write(7,'(2x,i10,3(2x,e24.16),2x,i10)') istmd,tstep,beta,       &
     &           tempk,iseed
         write(7,'(11(2x,e24.16))') press,((strext(i,j),i=1,3),j=1,3),   &
     &           celmas
         do nt = 1,ntype
           do i = 1,natom(nt)
             write(7,'(9(2x,e24.16))') (vat(j,i,nt),j=1,3),              &
     &                   (rat1(j,i,nt),j=1,3),(frc1(j,i,nt),j=1,3)
           enddo
         enddo
         write(7,'(9(2x,e24.16))') ((vadot(i,j),i=1,3),j=1,3)
         write(7,'(9(2x,e24.16))') ((adot1(i,j),i=1,3),j=1,3)
         write(7,'(9(2x,e24.16))') ((frcel1(i,j),i=1,3),j=1,3)
