...
!      start loop over atomic types
     ...
     &      nql,delql
         ...
         if (nql > mxdlqp) then
           write(6,'("  Stopped in in_rho_v_pseudo,  atom: ",a2,         &
     ...
     &       "in_rho_v_pseudo   chemical symbols do not match",          &
     ...
     &        "in_rho_v_pseudo:  correlation potential does not ",       &
...
!        read pseudopotentials
         ...
         if(irel(nt) == 'rel') then
           ...
           if(norb(1) > norb(0) .or. norb(-1) > norb(0)) then
             write(6,'("  WARNING in in_rho_v_pseudo,  orbitals:",       &
     &         i5,"  spin orbitals:",2i5)') norb(0),norb(-1),norb(1)
         ...
         if(norb(0) > 4) then
           write(6,'("  stopped in in_rho_v_pseudo,   atom:  ",          &
...
!          divides by 1/q**l
           ...
           if(l > 0) then
             ...
             vkbraw(0,l,-1,nt) = (4*vkbraw(1,l,-1,nt)                    &
     &                                  - vkbraw(2,l,-1,nt)) / 3
             vkbraw(0,l, 0,nt) = (4*vkbraw(1,l, 0,nt)                    &
     &                                  - vkbraw(2,l, 0,nt)) / 3
             vkbraw(0,l, 1,nt) = (4*vkbraw(1,l, 1,nt)                    &
     &                                  - vkbraw(2,l, 1,nt)) / 3
...
!        reads the fourier transforms of the wavefunctions
     ...
     &         nameat(nt),nqwf(nt)-1
         ...
         if(norbat(nt) > mxdlao) then
           write(6,'("  stopped in in_rho_v_pseudo,    atom ",           &
     &         a2,"  program does not accept ",i4,  " orbitals.",        &
     &         "  Maximum is: ",i4)') nameat(nt),norbat(nt),mxdlao
         ...
         do n=1,norbat(nt)
           read(io) lorb(n,nt),eorbwv
           if(lorb(n,nt)+1 > 4) then
             write(6,'("  stopped in in_rho_v_pseudo    atom ",          &
     &           a2,"  program does noes not accept l=",i4,              &
     &"          in wavefctions")') nameat(nt),lorb(n,nt)
...
!          divides by 1/q**l
           ...
           if(l > 0) then
             do j = 1,nqwf(nt)-1
               fac = (j*delqwf(nt))**l
               wvfao(j,n,nt) = wvfao(j,n,nt) / fac
             enddo

             wvfao(0,n,nt) = (4*wvfao(1,n,nt) - wvfao(2,n,nt)) / 3

           endif

           wvfao(-1,n,nt) = wvfao(1,n,nt)
           wvfao(-2,n,nt) = wvfao(2,n,nt)
