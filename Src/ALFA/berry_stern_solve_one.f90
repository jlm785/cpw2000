...
!>  Each state is treated individualy.
!>
...
subroutine berry_stern_solve_one(mtxd, neig, psi, ei, dhdkpsi, dpsi, tol,  &
    isort, ekpg,                                                         &
    vscr, kmscr,                                                         &
    ng, kgv,                                                             &
    nanl, anlga, xnlkb,                                                  &
      ...
      do m = 1,neig
        if(n /= m) then
          if(abs(ei(n)-ei(m)) < 0.0001) then
            write(6,*) '   stopped in berry_stern_solve_one.'
...
end subroutine berry_stern_solve_one
