!------------------------------------------------------------!
! This file is distributed as part of the md code and        !
! under the terms of the GNU General Public License. See the !
! file `LICENSE' in the root directory of the md             !
! distribution, or http://www.gnu.org/copyleft/gpl.txt       !
!                                                            !
! https://github.com/jlm785/md                               !
!------------------------------------------------------------!
...
!>  \version      5.13
!>  \date         January 2017. 17 March 2026.
...
! eosvol and eosepi. 17 March 2026. JLM
...
! local variables
  ...
  logical               ::  lfinisheos                                   !  finished calculating the equation of state E(V)
  logical               ::  lepi                                         !  if true, only adot(3,3) is scaled in move_eos
...
!     by noise
    ...
    elseif(flgcal == 'EOSVOL' .or. flgcal == 'EOSEPI') then

      call move_lbfgs_call(energy, rat, force, adot, stress,             &
          strext, press, ilbfgs, iconv, 1, 0,                            &
          ntype, natom,                                                  &
          mkeep, pgtol, dxmax,                                           &
          mxdtyp, mxdatm)
      ...
      elseif(iconv == 0) then
        ...
        if(flgcal == 'EOSVOL') then
          lepi = .FALSE.
        else
          lepi = .TRUE.
        endif

        call move_eos(energy, adot, lepi, lfinisheos)

        newcel = .TRUE.

        if(.not. lfinisheos) then
          ilbfgs = 0
          iconv = 0
