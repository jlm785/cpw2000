...
! prints calculation flags other information
...
! reads the first record from the file
  ...
  if(ng > mxdgve) then
    write(6,*)
    write(6,'("  STOPPED in in_v_rho")')
    write(6,'("  ng = ",i8," is greater than mxdgve =",i8)') ng, mxdgve
  ...
  if(ns > mxdnst) then
    write(6,*)
    write(6,'("  STOPPED in in_v_rho")')
    write(6,'("  ns = ",i8," is greater than mxdnst =",i8)') ns, mxdnst
  ...
  if(flgdal /= '    ') then
    if(flgdal == 'DUAL') then
      ...
      write(6,*)
      write(6,'("  The partial dual approximation was used ",a4)') flgdal
    endif
  endif
...
! backcompatibilty blues
  ...
  if(teleck > 0.1) then
    write(6,*)
    write(6,'("  The electronic temperature was ",f12.3," K")') teleck
  endif
  write(6,*)
