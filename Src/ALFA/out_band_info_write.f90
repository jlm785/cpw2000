...
! end binary write
  ...
  if(n10 > 0) then
    do j = 1,n10
      write(io,'(5x,10(l5,i5))') (ljump(jj),nkstep(jj),jj=10*(j-1)+1,10*j)
    enddo
  endif
  if(nlines > 10*n10) then
    write(io,'(5x,1010(l5,i5))') (ljump(jj),nkstep(jj),jj=10*n10+1,nlines)
  endif

  n10 = (nvert+nlines)/10
  if(n10 > 0) then
    do j = 1,n10
      write(io,'(5x,10(2x,a6,f14.6))') (label(jj),xklab(jj), jj=10*(j-1)+1,10*j)
    enddo
  endif
  if(nvert+nlines > 10*n10) then
    write(io,'(5x,1010(2x,a6,f14.6))') (label(jj),xklab(jj), jj=10*n10+1,nvert+nlines)
  endif

  write(io,'(5x,i8,5x,"number of atomic orbitals")') nbaslcao

  do n = 1,nbaslcao
    write(io,'(5x,5i8,5x,"type, atom, n, l ,m")') (infolcao(j,n),j=1,5)
  enddo
  ...
  do k = 1,nrk
    do n = 1,neig
      if(n10 > 0) then
        do j = 1,n10
          write(io,'(5x,10f14.6)') (basxpsi(jj,n,k), jj=10*(j-1)+1,10*j)
        enddo
      endif
      if(nbaslcao > 10*n10) then
          write(io,'(5x,10f14.6)') (basxpsi(jj,n,k), jj=10*n10+1,nbaslcao)
      endif
    enddo
  enddo
