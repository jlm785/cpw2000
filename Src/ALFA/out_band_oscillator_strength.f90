...
!>  Follows equations 6.51 and 6.41a of the Yu and Cardona
...
! local variables
  ...
  real(REAL64)      ::  spindeg                    !  spindegeneracy
...
! desired direction of oscillator strength
  ...
  if(lpair) then
    ...
    if(lxyz) then
      write(6,'(4x,"i",3x,"E_i(eV)",4x,"f",3x,"E_f(eV)",5x,              &
            &   "|<i|dH/dk|f>|**2",8x,"F_if",11x,                        &
            &   "|M_x|**2",4x,"|M_y|**2",4x,"|M_z|**2",7x,               &
            &   "|Fij_x|**2",2x,"|Fij_y|**2",2x,"|Fij_z|**2" )')
    else
      write(6,'(4x,"i",3x,"E_i(eV)",4x,"f",3x,"E_f(eV)",5x,              &
            &   "|<i|dH/dk|f>|**2",8x,"F_if",9x,"|M_dir|**2",9x,         &
            &   "|F_ij_dir|**2")')
    endif
    write(6,*)
    ...
    do j = 1,nfinal
      ...
      if(ii < jj .and. abs(efmei(ij)) > TOL) then

        if(lxyz) then
          write(6,'(i5,f10.3,i5,f10.3,6x,f12.5,6x,f12.5,4x,3f12.5,     &
              &   4x,3f12.5)')                                         &
              ii, ei(ii)*HARTREE, jj, ei(jj)*HARTREE,                  &
              real(ematif(ij)),                                        &
              (2.0/3.0)*real(ematif(ij))/efmei(ij),                    &
              (abs(ematcar(ij,k))**2,k=1,3),                           &
              (2*abs(ematcar(ij,k))**2/efmei(ij),k=1,3)
        else
          ematdirsq = abs(ematcar(ij,1)*rdirnorm(1) +                  &
                          ematcar(ij,2)*rdirnorm(2) +                  &
                          ematcar(ij,3)*rdirnorm(3))
          ematdirsq = ematdirsq*ematdirsq
          write(6,'(i5,f10.3,i5,f10.3,6x,f12.5,6x,f12.5,4x,f12.5,      &
              &   4x,f12.5)')                                          &
              ii, ei(ii)*HARTREE, jj, ei(jj)*HARTREE,                  &
              real(ematif(ij)),                                        &
              (2.0/3.0)*real(ematif(ij))/efmei(ij),                    &
              ematdirsq, 2*ematdirsq/efmei(ij)
        endif
...
!   fills the information
    ...
    do n = 1,nlevel
     ...
     if(abs(efmei(indx(leveigs(n,1)))) > TOL) then
        ...
        write(6,*)
        if(lxyz) then
          write(6,'(34x,"sum (incl. spin deg.) = ",f12.5,4x,3f12.5,4x,3f12.5)')     &
                spindeg*so, (spindeg*sm(k),k=1,3), (2*spindeg*sm(k)/efmei(ij),k=1,3)
        else
          write(6,'(34x,"sum (incl. spin deg.) = ",f12.5,4x,f12.5,4x,f12.5)')       &
                spindeg*so, spindeg*xsum, 2*spindeg*xsum/efmei(ij)
        endif
        write(6,*)
        write(6,*)
