...
!>  generate g_ij
!>
!>  \author       Alvaro Ladeira, Jose Luis Martins
!>  \version      4.94
!>  \date         1999, 2004, 18 August 2026.
...
! indentation, remove dble, 18 August 2026. JLM
...
! checks range of tol
  ...
  if(tol < 5*EPS) then
    scltol = 5*EPS
    if(iprint >= 1) write(6,*) 'metric_ident   tol= ',5*EPS
  endif



  if( cubrdetG < 10*scltol*scal) then
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
  ...
  do i=1,3
  do j=1,3
    G(i,j) = real(iG(i,j),REAL64)*scalint
  enddo
  enddo
...
!   ARCHES FOR CENTERED MONOCLINIC LATTICES
...
! makes sure that the determinant is 1
   ...
   icount = invm(1,1)*Mtotal(1,1) + invm(1,2)*Mtotal(2,1) + invm(1,3)*Mtotal(3,1)
