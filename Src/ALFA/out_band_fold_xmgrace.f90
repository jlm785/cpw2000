...
!>  Writes the unfolded band structure plot file for later use with xmgrace
!>  Usage:  xmgrace "filename", or just click on the agr file
!>
!>  \author       Carlos Loia Reis, Jose Luis Martins
!>  \version      5.12
!>  \date         19 October 2013, 6 August 2020.
...
! finds the energy range for the bands
  ...
  if(l > 0) then
    write(io,*)
    write(io,"('@    subtitle ""Unfolded ',a,'""')") adjustl(trim(subtitle))
    write(io,*)
  endif
  ...
  do n = 1,nvert+nlines
    ...
    elseif(adjustl(trim(label(n))) == '-' .or.                           &
      ...
      elseif(l == 3) then
        ...
        write(io,"('@    xaxis  ticklabel ',i5,' , ""',a10,'"" ')")      &
                       n-1,adjustl(trim(label(n)))
      endif
    endif
  enddo
...
!        write(io,*)
  ...
  enddo
    ...
    write(io,'("@type xy ")')
    write(io,'(2f18.8)') xk(1), 0.0
    write(io,'(2f18.8)') xk(nrk), 0.0
