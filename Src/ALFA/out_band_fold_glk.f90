...
!>   for the unfolded band structure using the generalized Luttinger-Kohn
!>   approximation.
!>
!>
...
subroutine out_band_fold_glk(diag_type, lworkers, xsvd, csvd,            &
...
! modified, interpolate_glk, cleaning, x/csvd. 17 September 2020. JLM
...
! allocatable for glk interpolation
  ...
  integer, allocatable               ::  irk_bound(:,:)
  integer, allocatable               ::  irk_map(:)
...
! local variables
  ...
  integer                   ::  io65, io64                               !  tape numbers
  logical                   ::  lex
...
! glk interpolation
  ...
  logical, parameter        ::  lkplusg = .false.
...
!!   this is a workaround  THIS MAYBE NO LONGER TRUE
...
! initialization avoids valgrind "errors"
  ...
  allocate(irk_bound(nrk2,2))
  allocate(irk_map(nvert))

  do irk = 1, nrk2

     do i=1, nvert-1
       if (xk(irk) >= xcvert(i)) then
         irk_bound(irk,1) = i
         irk_bound(irk,2) = i+1
         if (xk(irk) == xcvert(i)) then
           irk_map(i) = irk
...
! ratory calculation
  ...
  write(6,*)
  write(6,*) '  Preparatory calculation in the reference k-points'

  neig_all(1) = neig
  neig_all(2) = neig

  inquire(iolength = ir_size)  irk, rkpt, psi(:,:), mtxd, isort(:)

  open(unit = io65, file ="wf_bnd_rec.dat", access="direct", recl=ir_size)
...
! loop over k-points
  ...
  do irk=1,nrk2
    ...
    if(cworker== iworker) then
      irk_rd = -10
      read(io64,rec=irk, iostat=irec_err) irk_rd,                        &
      e_of_k(:,irk), pkn_tmp(:),                                         &
      ...
      if(irk_rd ==irk) then
        write(6,'("not computing k-point #",i5)') irk, irk_rd
        lmyjob = .FALSE.
      endif
    else
      write(6,'("not computing k-point #",i5)') irk, irk_rd
    ...
    if(lmyjob) then
      ...
      if(nd > mxddim) then
        write(6,'("mtxd has changed from ", i5, " to", i5)') mxddim, int(nd*1.01)
      ...
      read(io65,rec=irk_bound(irk,1))  irk_rd, rkpt_all(:,1),            &
          psi_all(:,:,1), mtxd_all(1), isort_all(:,1)

      read(io65,rec=irk_bound(irk,2))  irk_rd, rkpt_all(:,2),            &
          psi_all(:,:,2), mtxd_all(2), isort_all(:,2)

      m1  = irk_map(irk_bound(irk,2)) - irk_map(irk_bound(irk,1))

      zk(2) = 1.0*(irk-irk_map(irk_bound(irk,1)))/m1
      zk(1) = 1 - zk(2)

      call out_glk_interpolation(2, emax, neig, xsvd, csvd,              &
...
!--------------------------------------------------------------------
      call Fold_GetPknSO(pkn_so, iMinv, idet, irk, kgv, isort, psi_so,   &
          nrk2, 2*neig, mtxd, ng,                                        &
          mxdgve, 2*mxddim, 2*mxdbnd)
!--------------------------------------------------------------------


      write(6,'( "iworker #",i5, "   writing in irk # ",i5,              &
             &     "   of ", i5)') iworker, irk,nrk2
      write(6,*)
  ...
  call zeelap(t2)
  write(6,*)
  write(6,'(" elapsed time for preparation (s):", 2f14.3)') (t1-t0)
  write(6,'(" elapsed time for bands (s):", 2f14.3)') (t2-t1)
  write(6,*)

  if (pp_flag ==1) then
    write(6,*) 'Generating band structure files!'
  else
    close(io64)
    write(6,*) "this worker is done. run again when all workers ",       &
...
! if run by a human clean up temporary files
  ...
  if(.not. lworkers) then
    call execute_command_line("rm  band_fold_rec.dat 2> /dev/null ")
    call execute_command_line("rm  wf_bnd_rec.dat 2> /dev/null ")
  endif
  ...
  deallocate(irk_bound)
  deallocate(irk_map)

  return

end subroutine out_band_fold_glk
