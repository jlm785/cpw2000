       SUBROUTINE VPASS1(A,B,C,D,TRIGS,TRIGI,IFAC,LA,N,
     1  ISKIP,ISKIP1,ISKIP2,NUMBER,ISIGN)
C
...
CDEC$ IVDEP
             DO NU=0,NUMBER-1
               ...
               C(IADD2)   =A(IADD1)+T5REAL
               D(IADD2)   =B(IADD1)+T5IMAG
               C(IADD2+JB)=T8REAL-T10IMAG
               D(IADD2+JB)=T8IMAG+T10REAL
               C(IADD2+JC)=T9REAL-T11IMAG
               D(IADD2+JC)=T9IMAG+T11REAL
               C(IADD2+JD)=T9REAL+T11IMAG
               D(IADD2+JD)=T9IMAG-T11REAL
               C(IADD2+JE)=T8REAL+T10IMAG
               D(IADD2+JE)=T8IMAG-T10REAL
             ENDDO
             I=I+ISKIP
             J=J+ISKIP1
           ENDDO
...
CDEC$ IVDEP
               DO NU=0,NUMBER-1
                 ...
                 C(IADD2)   =A(IADD1)+T5REAL
                 D(IADD2)   =B(IADD1)+T5IMAG
                 C(IADD2+JB)=TRIGS(KK) *X1REAL-TRIGI(KK)*X1IMAG
                 D(IADD2+JB)=TRIGI(KK)*X1REAL+TRIGS(KK) *X1IMAG
                 C(IADD2+JC)=TRIGS(KK2)*X2REAL-TRIGI(KK2)*X2IMAG
                 D(IADD2+JC)=TRIGI(KK2)*X2REAL+TRIGS(KK2)*X2IMAG
                 C(IADD2+JD)=TRIGS(KK4)*X3REAL-TRIGI(KK4)*X3IMAG
                 D(IADD2+JD)=TRIGI(KK4)*X3REAL+TRIGS(KK4)*X3IMAG
                 C(IADD2+JE)=TRIGS(KK6)*X4REAL-TRIGI(KK6)*X4IMAG
                 D(IADD2+JE)=TRIGI(KK6)*X4REAL+TRIGS(KK6)*X4IMAG
               ENDDO
               I=I+ISKIP
               J=J+ISKIP1
             ENDDO
...
CDEC$ IVDEP
             DO NU=0,NUMBER-1
               ...
               C(IADD2)   =T1REAL+T2REAL
               D(IADD2)   =T1IMAG+T2IMAG
               C(IADD2+JB)=T3REAL-T4IMAG
               D(IADD2+JB)=T3IMAG+T4REAL
               C(IADD2+JC)=T1REAL-T2REAL
               D(IADD2+JC)=T1IMAG-T2IMAG
               C(IADD2+JD)=T3REAL+T4IMAG
               D(IADD2+JD)=T3IMAG-T4REAL
             ENDDO
             I=I+ISKIP
             J=J+ISKIP1
           ENDDO
...
CDEC$ IVDEP
             DO NU=0,NUMBER-1
               ...
               C(IADD2)   =A(IADD1)+T1REAL
               D(IADD2)   =B(IADD1)+T1IMAG
               C(IADD2+JB)=T2REAL-T3IMAG
               D(IADD2+JB)=T2IMAG+T3REAL
               C(IADD2+JC)=T2REAL+T3IMAG
               D(IADD2+JC)=T2IMAG-T3REAL
             ENDDO
             I=I+ISKIP
             J=J+ISKIP1
           ENDDO
...
CDEC$ IVDEP
           DO NU=0,NUMBER-1
             IADD1=IADD1+ISKIP2
             IADD2=NU+J
             C(IADD2)   =A(IADD1)+A(IADD1+IB)
             D(IADD2)   =B(IADD1)+B(IADD1+IB)
             C(IADD2+JB)=A(IADD1)-A(IADD1+IB)
             D(IADD2+JB)=B(IADD1)-B(IADD1+IB)
           ENDDO
           I=I+ISKIP
           J=J+ISKIP1
         ENDDO
...
CDEC$ IVDEP
             DO NU=0,NUMBER-1
               IADD1=IADD1+ISKIP2
               IADD2=NU+J
               X1REAL=A(IADD1)-A(IADD1+IB)
               X1IMAG=B(IADD1)-B(IADD1+IB)
               C(IADD2)   =A(IADD1)+A(IADD1+IB)
               D(IADD2)   =B(IADD1)+B(IADD1+IB)
               C(IADD2+JB)=TRIGS(KK)*X1REAL-TIMAG*X1IMAG
               D(IADD2+JB)=TIMAG*X1REAL+TRIGS(KK)*X1IMAG
             ENDDO
             I=I+ISKIP
             J=J+ISKIP1
           ENDDO
