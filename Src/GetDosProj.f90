...
! allocatable arrays
  ...
  real(REAL64), allocatable        ::  fgrid(:,:,:,:,:)                  !  eigenvalues in Hartree in regular grid
  real(REAL64), allocatable        ::  ehist(:)                          !  histogram energy
  real(REAL64), allocatable        ::  dhist(:)                          !  histogram dos
  real(REAL64), allocatable        ::  dhist_tot(:)                          !  histogram dos
  real(REAL64), allocatable        ::  chist(:)                          !  histogram integrated dos
  real(REAL64), allocatable        ::  chlow(:)                          !  histogram integrated dos
...
! constants
  ...
  integer :: nrk2, nband2, norb, iband, iorb

  open(unit=190,file="dos_file_proj.dat", form="unformatted")
  read(190)  nrk2
  read(190)  nband2
  read(190)  norb
  allocate (basxpsi_of_k(norb,norb,nrk2))
  do irk=1,nrk2
    read(190) basxpsi_of_k(:,:,irk)
  enddo
...
! following lines do not depend on change of data file format
  ...
  allocate(fgrid(nx(1),nx(2),nx(3),mxdbnd,norb))

  call dos_el_to_egrid(el,nrk,nband,nx,kmap,egrid,nx,mxdbnd)

!  allocate (basxpsi_of_k(norb,nband2,nrk2))

  do iorb=1,norb
    call dos_el_to_egrid(basxpsi_of_k(iorb,1:mxdbnd,:),nrk,nband,nx,kmap,fgrid(:,:,:,:,iorb),nx,mxdbnd)
  enddo
  ...
  if(ichoice == 3) then
    ...
    if(yesno == 'y' .or. yesno == 'Y') then
      ...
      open(unit=191,file="dos.bv", form="unformatted")
      
      write(191) mxdbnd, norb, nhist
      write(191) ehist
      write(191) dhist   ! first entry is total dos
      
                        
      do iorb=1, norb  ! loop over all projections
        dhist_tot(:) = ZERO
        do iband=1, mxdbnd
          call dosf_lin_one(nx,egrid(:,:,:,iband),fgrid(:,:,:,iband,iorb),nhist,ehist,dhist,ezero,ispin,  &
               lunif,.TRUE.,vol)               
          dhist_tot(:) = dhist_tot(:) +  dhist(:)
        enddo

        write(*,'("processing orbital", i5, " of", i5)') iorb, norb
        write(191) dhist_tot
      enddo
