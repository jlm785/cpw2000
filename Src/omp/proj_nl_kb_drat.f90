...
!>  and the respective derivatives with respect to
...
subroutine proj_nl_kb_drat(rkpt, mtxd, isort, nanl,                      &
    ng, kgv,                                                             &
    nqnl, delqnl, vkb, nkb,                                              &
    ntype, natom, rat, adot,                                             &
    anlga, xnlkb, danldrat, k_ind, kk_ind,                               &
    mxdtyp, mxdatm, mxdlqp, mxddim, mxdanl, mxdgve)

! adapted from proj_nl_kb_c16
...
! output
  ...
  complex(REAL64), intent(out)       ::  danldrat(3,mxddim,mxdanl)       !<  d anl / d rat
  integer, intent(out)               ::  k_ind(mxdanl)                   !<  index of atom type of projector
  integer, intent(out)               ::  kk_ind(mxdanl)                  !<  index of atom of a given type of projector

! allocatable local variables

  complex(REAL64), allocatable  ::  st(:,:)
  complex(REAL64), allocatable  ::  dstdrat(:,:,:)
...
! paranoid check, stops complaint about unused ng...
  ...
  allocate(dstdrat(3,mxdatm,mxdtyp))
  allocate(vqil(0:LMAX,mxdtyp))

  allocate(l_ind(nanl), m_ind(nanl))
...
!$omp& shared(nanl, anlga, danldrat, k_ind, kk_ind, l_ind, m_ind)
  do i = 1,mtxd
...
!   structure phase factor
    ...
    do k = 1,ntype
      do kk = 1,natom(k)
        ...
        dstdrat(1,kk,k) = 2*PI*cmplx(sin(fi),-cos(fi),REAL64)*kgv(1,isort(i))
        dstdrat(2,kk,k) = 2*PI*cmplx(sin(fi),-cos(fi),REAL64)*kgv(2,isort(i))
        dstdrat(3,kk,k) = 2*PI*cmplx(sin(fi),-cos(fi),REAL64)*kgv(3,isort(i))
      enddo
    enddo
...
!   starts loop over second index
    ...
    do ind = 1,nanl
      ...
      danldrat(1,i,ind) = dstdrat(1,kk,k)*rylm(l,m)*vqil(l,k)
      danldrat(2,i,ind) = dstdrat(2,kk,k)*rylm(l,m)*vqil(l,k)
      danldrat(3,i,ind) = dstdrat(3,kk,k)*rylm(l,m)*vqil(l,k)
...
! sign of projector normalization
  ...
  deallocate(st, dstdrat)
  deallocate(vqil)

  deallocate(l_ind, m_ind)

  return
end subroutine proj_nl_kb_drat
