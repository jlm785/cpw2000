...
!        update inverse jacobian matrix h - p x hy
         ...
         if (icount > 0) then
           ...
           vnorm1 = ZERO
           do i = 2,nj
             vnorm1 = vnorm1 + real((vhxc(i)-vold(i)) *                  &
     &                                 conjg(dgrfi(i)),REAL64)
           enddo

           vnorm2 = ZERO
           do i = 2,nj
             vnorm2 = vnorm2 + real(hy(i,icount) *                       &
     &                                conjg(dgrfi(i)),REAL64)
           enddo
...
!            renormalizes stuff
             ...
             do i = 2,nj
               uvec(i,icount) = pvec(i,icount) / vnorm1                  &
     &                         - hy(i,icount) / vnorm2
             enddo
...
!        compute correction to vhxc from inverse jacobian
         ...
         if(icount > 0) then
           do k = 1,icount
             ...
             xsum = ZERO
             do j=2,nj
               xsum = xsum + real(hy(j,k) *                              &
     &           conjg(vhxcout(j)-vhxc(j)),REAL64)
             enddo
