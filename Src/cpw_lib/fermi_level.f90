...
!      temperature added 25 nov 90.rmmel(mxdbnd*mxdnrk) 
         ...
         do i=1,nrk
           do j=1,nband(i)
             ...
             if (frac(iel) > SMALL) then
               elects = elects - 2 * tempau * wgk(i) *                   & 
     &              frac(iel) * log(frac(iel))
             endif
             if ((UM - frac(iel)) > SMALL) then
               elects = elects - 2 * tempau * wgk(i) *                   &
     &               (UM - frac(iel)) * log(UM - frac(iel))
             endif
           enddo
         enddo
       endif
...
!      estimates the penn gap
       ...
       penngap = ZERO
       jmax = nint(ztot/(2*UM))
       if(abs(ztot - real(2*jmax)) < EPS) then
         imx = 0
         iel = 0
         do i=1,nrk
