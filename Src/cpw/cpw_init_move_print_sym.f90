...
!>  Initial stuff, including initialization of move, printing
!>  initial parameters, identify symmetry.
!>
!>  \author       Jose Luis Martins
!>  \version      5.13
!>  \date         before November 2019. 31 March 2026.
...
! Written before 19 November 2019. JLM
! Indentation, filename in move_restart_in. 31 March 2026. JLM
  use cpw_variables
...
! Random initial velocities
  ...
  call move_random_v_atom(moldyn_%iseed, moldyn_%tempinik, moldyn_%vat,  &
      vcsdyn_%vadot, crys_%ntype, crys_%natom, crys_%atmass, crys_%adot, &
      dims_%mxdtyp,dims_%mxdatm)

  if(flags_%flgcal == 'RSTRT ') then

    call move_restart_in(flags_%flgcal, 7, 'RESTART.DAT',                &
        crys_%ntype, crys_%natom, crys_%nameat, crys_%atmass, crys_%rat, &
        moldyn_%vat, crys_%adot, vcsdyn_%vadot,                          &
        ist0, moldyn_%tstep, moldyn_%beta, moldyn_%tempk, moldyn_%iseed, &
        vcsdyn_%strext, vcsdyn_%press, vcsdyn_%celmas,                   &
  ...
  call print_parameters(flags_%flgcal, flags_%flgdal, flags_%flgscf,     &
      pwexp_%emax, xc_%author, xc_%tblaha,                               &
      acc_%epscv, acc_%epscvao, acc_%epspsi,                             &
      moldyn_%tempk, pwexp_%teleck, moldyn_%tempinik, moldyn_%nstep,     &
      moldyn_%tstep, moldyn_%beta, moldyn_%iseed, vcsdyn_%press,         &
      vcsdyn_%strext, vcsdyn_%celmas, pwexp_%lkplusg, pwexp_%epskplusg)
