...
!         avoids restarting from same place

          do i=1,recip_%ns
            vhxc(i) = (UM-pmix)*vhxclow(i) + pmix*vhxc(i)
          enddo
          ...
          call pot_local(ipr, vscr, vmax, vmin, vcomp_%veff, kmscr, idshift,  &
            ...
            write(6,'("   STOPPED in cpw_scf:  cycle is diverging",      &
               &      " local potential jumped by",g14.6)')              &
                      max(abs(vmin-vminold),abs(vmax-vmaxold))
            write(6,'("   and maximum number of restarts exceeded")')
...
!           avoids restarting from same place
            ...
            enddo
            do i = 1,recip_%ns
              vhxcout(i) = (UM-pmix)*vhxcoutlow(i) + pmix*vhxcout(i)
            enddo
