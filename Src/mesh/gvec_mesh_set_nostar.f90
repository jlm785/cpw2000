...
!>  Distributes a quantity in G-space (not on the prototype)
!>  in an FFT mesh
!>
!>  \author       Carlos Loia Reis, José Luís Martins
!>  \version      5.13
!>  \date         September 30 2015, 19 August 2026.
!>  \copyright    GNU Public License v2

subroutine gvec_mesh_set_nostar(ipr, purpose, adot, deng,                &
    rhomsh, id,n1,n2,n3, lvol,                                           &
    ng, kgv, kmax,                                                       &
    mxdgve, mxdscr)

! Written 19 August 2026, from gvec_mesh_set. JLM
...
! so it is safer to call size_fft again
  ...
  if(nsfft(1) /= n1 .or. nsfft(2) /= n2 .or. nsfft(3) /= n3              &
      .or. id < n1) then
    write(6,*)
    write(6,*)  "  STOPPED in gvec_mesh_set_nostar applied to ", purpose
  ...
  if(mxdfft > mxdscr) then
    write(6,*)
    write(6,'("   STOPPED in gvec_mesh_set_nostar.  mxdfft = ",i8,        &
  ...
  if(ipr > 2) then
    write(6,*)
    write(6,'("  gvec_mesh_set_nostar  n = ",3i5)') n1,n2,n3
    write(6,*)
  endif
...
! initialize charge density array and enter symmetrized
  ...
  lwrap = .TRUE.
  if(kmax(1) < kmfft(1) .and. kmax(2) < kmfft(2) .and.                   &
      kmax(3) < kmfft(3)) then
...
! and converts to electrons(whatever)/bohr^3
  ...
  if (ierr /= 0) then
    write(6,*)
    write(6,*) '    WARNING in gvec_mesh_set_nostar for ', purpose
  ...
  if (cmax > 1000.0*EPS) then
    write(6,*)
    write(6,*) '   STOPPED in gvec_mesh_set_nostar for ', purpose
...
end subroutine gvec_mesh_set_nostar
